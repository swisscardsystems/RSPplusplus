...
with token.open(user_pin=user_pin) as session:
    ...
    server_pub_key = session.get_key(label='DH_KEY_SERVER', object_class=ObjectClass.PUBLIC_KEY)
    raw_pub_bytes = server_pub_key[Attribute.EC_POINT]

    with open("server_pub.bin", "wb") as file:
